...
func make_piece(quantity):
	...
	print("GB ", quantity, " pieces made and added to all_pieces")
	pass
...
func place_piece(piece_id, new_cell, old_cell):
	...
	all_pieces[piece_id].position = to_local(cell_location[new_cell])
	#piece position is off just a little bit. 
	all_pieces[piece_id].position = all_pieces[piece_id].position + Vector2(5,35)
...
func _on_activate_generator(id):
	...
	#all_pieces[piece_id].get_node("active").position = to_local(Vector2(all_pieces[game_board[id]].position))
	#all_pieces[piece_id].get_node("active").position = all_pieces[game_board[id]].position
	all_pieces[piece_id].get_node("active").position = to_local(cell_location[cell_id])
	#piece position is off just a little bit. 
